...
class DataforIndividual():
    ...
    def load_forcing_data(self, working_path, huc_id, basin_id):
        forcing_path = os.path.join(working_path, 'gw_camelsgruadd', 'basin_mean_forcing', 'daymet', huc_id,
    ...
    def load_flow_data(self, working_path, huc_id, basin_id, area):
        flow_path = os.path.join(working_path, 'gw_camelsgruadd', 'usgs_streamflow', huc_id,
    ...
    def load_data(self):
        ...
        merged_data = merged_data[(merged_data['date'] >= datetime(1980, 9, 1)) &
                                  (merged_data['date'] <= datetime(2014, 9, 30))]
        merged_data = merged_data.set_index('date')
        pd_data = merged_data[['prcp(mm/day)', 'tmean(C)', 'dayl(day)', 'srad(W/m2)', 'vp(Pa)', 'flow(mm)', 'GW(feet)' ]]
